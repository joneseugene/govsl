...
export default async function PressReleaseSection() {
  const announcement_data: AnnouncementInterface[] = await getAnnouncements();

  const appointment_data: AppointmentInterface[] = await getAppointments();

  const news_data: NewsArticleInterface[] = await getNewsArticles();

  const press_release_data: PressReleaseInterface[] = await getPressReleases();

  const publication_data: PublicationInterface[] = await getPublications();

  const mda_data: MDAInterface[] = await getMdas();

  const service_data: ServicesInterface[] = await getServices();

  const service_detail_data: ServiceDetailsInterface[] = await getServiceDetails();
  ...
  return (
    <div className="space-y-10">
        ...
        {announcement_data?.length ? (
          <div className="grid gap-4">
            {announcement_data.map((item) => (
              <div key={item.id} className="p-4 border rounded-lg hover:shadow-sm transition">
      ...
      {/* ===================== PRESS RELEASE ===================== */}
      <section>
        <h2 className="text-xl font-bold text-gray-800 mb-4">Press Release</h2>
        ...
        {press_release_data?.length ? (
          ...
          <p className="text-sm text-gray-500">No press release available.</p>
        )}
      </section>
      ...
      <section>
        ...
        {publication_data?.length ? (
          ...
          <p className="text-sm text-gray-500">No publication available.</p>
        )}
      </section>

      {/* ===================== MDA ===================== */}
      <section>
        <h2 className="text-xl font-bold text-gray-800 mb-4">MDA</h2>

        {mda_data?.length ? (
          <div className="grid gap-4">
            {mda_data.map((item) => (
              <div key={item.id} className="p-4 border rounded-lg hover:shadow-sm transition">
                <h3 className="font-semibold text-gray-900">{item.name ?? 'Untitled'}</h3>
                <p className="text-sm text-gray-600 mt-1">
          ...
          <p className="text-sm text-gray-500">No mda available.</p>
        )}
      </section>
      ...
      {/* ===================== SERVICE DETAIL ===================== */}
      <section>
        <h2 className="text-xl font-bold text-gray-800 mb-4">Service Detail</h2>
        ...
        {service_detail_data?.length ? (
          <div className="grid gap-4">
            {service_detail_data.map((item) => (
              <div key={item.id} className="p-4 border rounded-lg hover:shadow-sm transition">
                <h3 className="font-semibold text-gray-900">{item.name ?? 'Untitled'}</h3>
                <p className="text-sm text-gray-600 mt-1">{item.price ?? 'No price available'}</p>
                <p className="text-sm text-gray-600 mt-1">
                  {item.description ?? 'No description available'}
                </p>
              </div>
          ...
          <p className="text-sm text-gray-500">No service detail available.</p>
        )}
      </section>
    </div>
  );
